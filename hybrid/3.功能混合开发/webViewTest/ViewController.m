...
//    NSLog(@"%@", path);
    ...
    NSRange range = [path rangeOfString:@"felixlu://"];
...
//    NSString *method = [NSString stringWithFormat:@"var img = document.getElementById(\"img\"); img.src = '%@';", path];
    NSString *method = [NSString stringWithFormat:@"wx.nativeCallback('takePicture', {status: 'success', value: '%@'})", path];
//    NSLog(@"%@", method);
    //向webview窗口注入js代码
...
- (void)getRQCode
{
    ...
    rqCodeVC.myBlock = ^(NSString *info){
        NSString *logInfo = [NSString stringWithFormat:@"wx.nativeCallback('getRQCode', {status: 'success', value: '%@'})", info];
//        NSLog(@"%@", logInfo);
